...
<div class="max-w-6xl mx-auto space-y-10 py-6"> 
    ...
    @auth
        ...
        <div class="space-y-6">
            <div class="border-l-4 border-blue-600 pl-4">
                <h2 class="text-2xl font-black text-gray-900 tracking-tight uppercase">Dashboard Siswa</h2>
                <p class="text-gray-400 text-[10px] font-bold uppercase tracking-[0.2em]">Ringkasan aktivitas kamu</p>
            </div>

            <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                <div class="bg-white p-6 rounded-[2rem] border border-gray-100 shadow-sm flex items-center space-x-4 transition-all hover:shadow-md">
                    <div class="p-4 bg-blue-50 text-blue-600 rounded-2xl">
                        <i class="ph-bold ph-books text-2xl"></i>
                    </div>
                    <div>
                        <p class="text-[10px] font-black text-gray-400 uppercase tracking-widest">Buku Dipinjam</p>
                        <h3 class="text-2xl font-black text-gray-900">{{ $peminjamanAktif->count() }}</h3>
                    </div>
                </div>

                <div class="bg-white p-6 rounded-[2rem] border border-gray-100 shadow-sm flex items-center space-x-4 transition-all hover:shadow-md">
                    <div class="p-4 {{ $totalDendaUser > 0 ? 'bg-red-50 text-red-600 animate-pulse' : 'bg-emerald-50 text-emerald-600' }} rounded-2xl">
                        <i class="ph-bold ph-hand-coins text-2xl"></i>
                    </div>
                    <div>
                        <p class="text-[10px] font-black text-gray-400 uppercase tracking-widest">Tanggungan Denda</p>
                        <h3 class="text-2xl font-black {{ $totalDendaUser > 0 ? 'text-red-600' : 'text-emerald-600' }}">
                ...
                <div class="bg-white p-6 rounded-[2rem] border border-gray-100 shadow-sm flex items-center space-x-4 transition-all hover:shadow-md">
                    ...
                    <div>
                        <p class="text-[10px] font-black text-gray-400 uppercase tracking-widest">Status Anggota</p>
                        <h3 class="text-lg font-black text-gray-900 uppercase tracking-tight">{{ Auth::user()->role }}</h3>
                    </div>
                </div>
            </div>
        </div>
    ...
    <div class="space-y-8 pt-4">
        <div class="flex flex-col md:flex-row justify-between items-end gap-6 border-b border-gray-100 pb-8">
            <div>
                <h2 class="text-3xl font-black text-gray-900 tracking-tight">Eksplorasi Buku</h2>
                <p class="text-gray-500 font-medium text-sm mt-1">
                    @if(request('kategori'))
                        Menampilkan kategori <span class="text-blue-600 font-bold uppercase">"{{ request('kategori') }}"</span>
                    @else
            ...
            <div class="flex flex-wrap gap-2">
                <a href="{{ route('home') }}" 
                   class="px-5 py-2.5 rounded-xl text-[10px] font-black transition-all uppercase tracking-tighter {{ !request('kategori') ? 'bg-blue-600 text-white shadow-lg' : 'bg-gray-100 text-gray-400 hover:bg-gray-200' }}">
                   Semua Buku
                </a>
                ...
                @foreach($kategoris as $kat)
                    <a href="{{ route('home', ['kategori' => $kat]) }}" 
                       class="px-5 py-2.5 rounded-xl text-[10px] font-black transition-all uppercase tracking-tighter {{ request('kategori') == $kat ? 'bg-blue-600 text-white shadow-lg' : 'bg-gray-50 text-gray-400 hover:bg-gray-100' }}">
                        {{ $kat }}
                    </a>
                @endforeach
            </div>
        ...
        {{-- 3. GRID KOLEKSI BUKU (DITAMBAHKAN CLASS SEARCHABLE-ITEM) --}}
        <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-6">
            @forelse($books as $book)
            {{-- Class 'searchable-item' penting untuk fitur search di sidebar --}}
            <a href="{{ route('daftar_buku.show', $book->id) }}" 
               class="searchable-item group relative aspect-[1/1.5] overflow-hidden rounded-[2rem] shadow-sm transition-all duration-500 hover:-translate-y-2 hover:shadow-2xl bg-gray-100 block">
            ...
            <div class="col-span-full py-20 text-center bg-gray-50 rounded-[3rem] border-2 border-dashed border-gray-100">
                <p class="text-gray-400 text-sm font-black uppercase tracking-widest italic">Buku tidak ditemukan.</p>
            </div>
            @endforelse
        </div>
    </div>
